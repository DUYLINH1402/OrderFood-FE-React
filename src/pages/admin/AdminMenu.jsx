...
const AdminMenu = () => {
  ...
  }, [isAdmin, fetchInitialData, fetchStats]);
  ...
  // Fetch foods khi page/pageSize hoặc filter thay đổi
  useEffect(() => {
    if (isAdmin && activeTab === TABS.MENU) {
  ...
  // Foods đã được filter từ server
    ...
    try {
      ...
      if (formData.image) {
        try {
          ...
          console.log("Không thể upload ảnh:", uploadErr.message);
          toast.warning("Không thể upload ảnh, sử dụng ảnh cũ");
        }
      }
      ...
      if (selectedFood) {
        ...
        // Update local state
        setFoods((prev) =>
          prev.map((food) => (food.id === selectedFood.id ? { ...food, ...foodData } : food))
        );
      } else {
        // Create
      ...
      const errorMessage = err.response?.data?.message || "Có lỗi xảy ra khi lưu món ăn";
  ...
  const handleDeleteConfirm = async (foodId) => {
    try {
      ...
      const errorMessage = err.response?.data?.message || "Có lỗi xảy ra khi xóa món ăn";
      toast.error(errorMessage);
    } finally {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center p-6 tablet:p-8 bg-white rounded-xl shadow-lg max-w-md mx-4">
          <FiAlertCircle className="w-14 h-14 tablet:w-16 tablet:h-16 text-red-500 mx-auto mb-3 tablet:mb-4" />
  ...
  const renderFoodCardGrid = (food) => {
    ...
    return (
      <div
        ...
        {/* Image */}
        <div
          className="relative h-36 tablet:h-44 desktop:h-48 bg-gray-100 overflow-hidden cursor-pointer"
          onClick={() => {
            setSelectedFood(food);
        ...
        <div className="p-3 tablet:p-4">
          ...
          <div className="mb-2">
            <span className="inline-flex items-center px-1.5 tablet:px-2 py-0.5 tablet:py-1 bg-gray-100 text-gray-600 rounded text-sx tablet:text-sm max-w-full">
              <FiPackage className="w-6 h-6 mr-1 flex-shrink-0" />
          ...
          <div className="flex items-center justify-between gap-4 mb-4">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-lg tablet:text-lg desktop:text-lg text-orange-600">
            ...
            {food.totalSold !== undefined && food.totalSold !== null && (
              <div className="flex items-center  gap-3 bg-orange-50 px-3 py-2 tablet:px-4 tablet:py-2.5 rounded-2xl border border-orange-100 transition-transform hover:scale-105">
                <div className="flex">
                  <span className="block text-sx tablet:text-sx text-gray-500 tracking-wider font-medium">
                    Đã bán
                  </span>
                  <p className="font-bold text-gray-900 text-sm tablet:text-base leading-none ml-1.5">
                    {food.totalSold?.toLocaleString("vi-VN")}
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="flex gap-1.5 tablet:gap-2">
            <button
              ...
              className={`flex-1 px-2 tablet:px-3 py-1.5 tablet:py-2 rounded-lg text-sx tablet:text-sm font-medium transition-colors flex items-center justify-center ${
              ...
              {food.status === "AVAILABLE" ? (
                <>
                  <FiAlertCircle className="w-6 h-6 mr-1" />
                  <span className="hidden tablet:inline">Báo hết</span>
                </>
              ) : (
                <>
                  <FiCheckCircle className="w-6 h-6 mr-1" />
                  <span className="hidden tablet:inline">Báo có</span>
                </>
              )}
            </button>
            <button
              onClick={() => handleOpenEditModal(food)}
              className="px-2 tablet:px-3 py-1.5 tablet:py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 text-sx tablet:text-sm font-medium transition-colors flex items-center justify-center">
  ...
  const renderFoodCardList = (food) => {
    ...
    return (
      <div
        key={food.id}
        className="bg-[#edf3eb96] border border-gray-400 rounded-xl p-4 hover:shadow-md transition-all duration-300 flex gap-4">
        {/* Image */}
        <div
          className="relative w-20 h-20 tablet:w-28 tablet:h-28 desktop:w-32 desktop:h-32 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100 cursor-pointer hover:opacity-90 transition-opacity"
          onClick={() => {
            setSelectedFood(food);
            setShowDetailModal(true);
          }}>
      ...
      <div className="mx-auto px-4 py-6">
        ...
        <div className="flex flex-col tablet:flex-row tablet:items-center tablet:justify-between mb-6">
          <div>
            ...
            <p className="text-sm tablet:text-md desktop:text-base text-gray-600">
              Thêm, sửa, xóa và quản lý trạng thái món ăn
            </p>
            {lastUpdated && (
          ...
          <div className="flex gap-2 mt-4 tablet:mt-0">
            <button
              ...
              className="inline-flex items-center justify-center px-3 tablet:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors text-sm tablet:text-md desktop:text-base">
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center justify-center px-3 tablet:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 text-sm tablet:text-md desktop:text-base">
          ...
          <div className="bg-white rounded-xl p-3 tablet:p-4 shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 tablet:gap-3">
              <div className="w-8 h-8 tablet:w-10 tablet:h-10 bg-blue-100 rounded-full flex items-center justify-center">
                <FiPackage className="w-5 h-5 tablet:w-5 tablet:h-5 text-blue-600" />
              </div>
              <div>
                {statsLoading ? (
          ...
          <div className="bg-white rounded-xl p-3 tablet:p-4 shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 tablet:gap-3">
              <div className="w-8 h-8 tablet:w-10 tablet:h-10 bg-yellow-100 rounded-full flex items-center justify-center">
                <FiAlertCircle className="w-5 h-5 tablet:w-5 tablet:h-5 text-yellow-600" />
              </div>
              <div>
                {statsLoading ? (
                  <div className="h-7 w-12 bg-gray-200 rounded animate-pulse" />
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
          <div className="flex border-b border-gray-200 overflow-x-auto">
            <button
              ...
              <FiList className="w-5 h-5 tablet:w-5 tablet:h-5 mr-1.5 tablet:mr-2" />
            ...
            <button
              ...
              <FiTrendingUp className="w-5 h-5 tablet:w-5 tablet:h-5 mr-1.5 tablet:mr-2" />
              <span className="hidden tablet:inline">Món bán chạy / Đặc biệt</span>
              <span className="tablet:hidden">Nổi bật</span>
            </button>
            <button
              ...
              <FiPercent className="w-5 h-5 tablet:w-5 tablet:h-5 mr-1.5 tablet:mr-2" />
              Khuyến mãi
            </button>
          </div>
            ...
            {loading ? (
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 tablet:p-4">
                <div className="flex items-center">
                  <FiAlertCircle className="w-5 h-5 tablet:w-5 tablet:h-5 text-red-500 mr-2" />
                  ...
                  <>
                    ...
                    <div className="mb-4 tablet:mb-6">
                      <button
                        ...
                        <FiFilter className="w-5 h-5 tablet:w-5 tablet:h-5 mr-1.5 tablet:mr-2" />
                        Bộ lọc
                        {showFilters ? (
                          <FiChevronUp className="w-5 h-5 tablet:w-5 tablet:h-5 ml-1" />
                        ) : (
                          <FiChevronDown className="w-5 h-5 tablet:w-5 tablet:h-5 ml-1" />
                        )}
                      </button>

                      {showFilters && (
                        <div className="grid grid-cols-1 items-center tablet:grid-cols-2 desktop:grid-cols-4 gap-3 tablet:gap-4 p-3 tablet:p-4 bg-gray-50 rounded-lg">
                          {/* Search */}
                          <Input
                            placeholder="Tìm kiếm tên món..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            prefix={<FiSearch className="text-gray-400 w-6 h-6" />}
                            allowClear
                            size="large"
                            className="w-full"
                          />
                    ...
                    {foodsLoading ? (
                      ...
                      <p className="text-sx tablet:text-sm desktop:text-md text-gray-600 mb-4">
