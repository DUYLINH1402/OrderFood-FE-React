...
import {
  getSuperAdminBlogsApi,
  deleteSuperAdminBlogApi,
  updateSuperAdminBlogStatusApi,
  getSuperAdminCategoriesApi,
} from "../../services/api/superAdminApi";
import { toggleBlogProtected } from "../../services/service/superAdminService";
...
const AdminBlogs = () => {
  ...
  const getBlogsApi = isSuperAdmin ? getSuperAdminBlogsApi : getAdminBlogsApi;
  const deleteBlogApi = isSuperAdmin ? deleteSuperAdminBlogApi : deleteAdminBlogApi;
  const updateStatusApi = isSuperAdmin ? updateSuperAdminBlogStatusApi : updateBlogStatusApi;
  const getCategoriesApi = isSuperAdmin ? getSuperAdminCategoriesApi : getAdminCategoriesApi;
  ...
  const handleStatusChange = async (blog, newStatus) => {
    try {
      const response = await updateStatusApi(blog.id, newStatus);
      if (response.success) {
  ...
  const handleDeleteBlog = (blog) => {
    confirm({
      ...
      onOk: async () => {
        try {
          const response = await deleteBlogApi(blog.id);
          if (response.success) {
  ...
  const handleToggleProtected = async (blog) => {
    if (!isSuperAdmin) return;
    const newProtected = !blog.isProtected;
    try {
      const result = await toggleBlogProtected(blog.id, newProtected);
      if (result.success) {
        setBlogs((prev) =>
          prev.map((b) => (b.id === blog.id ? { ...b, isProtected: newProtected } : b))
        );
        toast.success(
          newProtected ? `Đã bật bảo vệ cho "${blog.title}"` : `Đã tắt bảo vệ cho "${blog.title}"`
        );
      } else {
                    ...
                    <tr key={blog.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          {isSuperAdmin && (
                            <button
                              onClick={() => handleToggleProtected(blog)}
                              className={`p-1 ${
                                blog.isProtected
                                  ? "text-purple-600 hover:text-purple-900"
                                  : "text-gray-400 hover:text-purple-600"
                              }`}
                              title={blog.isProtected ? "Bỏ bảo vệ" : "Bảo vệ bài viết"}>
                              <svg
                                className="w-6 h-6"
                                fill="none"
                                stroke="currentColor"
